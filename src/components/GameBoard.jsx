...
function GameBoard({ farcasterUser }) {
  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      if (farcasterUser) {
        const hubClient = new HubClient({ hubUrl: 'https://relay.farcaster.xyz' });
        ...
        if ((state >> 19) & 0x3 !== 2) {
          ...
          setGameResult(
            winner === ethers.constants.AddressZero
              ? "It's a draw!"
              : `Winner: ${winner === farcasterUser.address ? 'You' : 'Opponent'}!`
          );
        }
      }
    };
    fetchGameState();
  }, [gameId, contract, farcasterUser]);
  ...
  const handleCommitMove = async (position) => {
    if (board[position] !== 0 || currentPlayer !== farcasterUser.address || gameResult) {
      return;
    }
    const salt = ethers.utils.hexlify(ethers.utils.randomBytes(32));
    const moveHash = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(['uint8', 'bytes32'], [position, salt]));
    try {
  ...
  const handleRevealMove = async () => {
    try {
      const tx = await contract.revealMove(gameId, movePosition, moveSalt);
      await tx.wait();
      setIsCommitting(false);
      const state = await contract.games(gameId);
      const gamePlayers = await contract.players(gameId);
  ...
  const handleTimeout = async () => {
    try {
      ...
      const gamePlayers = await contract.players(gameId);
      const winner = (state >> 18) & 0x1 === 0 ? gamePlayers[1] : gamePlayers[0];
      setGameResult(`Timed out! Winner: ${winner === farcasterUser.address ? 'You' : 'Opponent'}!`);
    } catch (error) {
    ...
    return (
      <button
        ...
        className={`w-20 h-20 bg-gray-900 text-4xl font-bold flex items-center justify-center border-2 border-purple-500 rounded-lg transition-all duration-300 ${
          isX ? 'text-purple-400 shadow-[0_0_15px_rgba(147,51,234,0.7)] animate-glow' : 
          isO ? 'text-pink-400 shadow-[0_0_15px_rgba(244,114,182,0.7)] animate-glow' : 
          'hover:bg-gray-800 hover:shadow-[0_0_15px_rgba(147,51,234,0.5)]'
        }`}
        disabled={value !== 0 || currentPlayer !== farcasterUser.address || gameResult}
      >
        {isX ? 'X' : isO ? 'O' : ''}
      </button>
    );
  ...
  return (
    <div className="w-full max-w-md p-6 bg-gray-900 bg-opacity-80 rounded-xl shadow-2xl border-2 border-purple-500 animate-glow">
      <h2 className="text-3xl font-bold text-purple-400 mb-6 text-center animate-neon">Tic-Tac-Toe PvP</h2>
      <div className="mb-6">
      ...
      {gameId && (
        <div className="mb-6">
          <h3 className="text-xl font-semibold text-purple-300 mb-4 text-center animate-neon">Game #{gameId}</h3>
          ...
          <p className="text-purple-200 mt-4 text-center animate-glow">
            {gameResult ? gameResult : `Current Player: ${currentPlayer === farcasterUser.address ? 'You' : 'Opponent'}`}
          </p>
          {!gameResult && (
            <button
